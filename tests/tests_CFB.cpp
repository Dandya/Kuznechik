...
TEST(CFB, TestFromGOST) {
    ...
    EXPECT_EQ(0, encryptCFBKuz(open_text, close_text, key, 16, 32, (uint8_t *)initial_vector));
    fclose(open_text);
    fclose(close_text);
    ...
    close_text = fopen("CloseText.txt", "r");
    EXPECT_EQ(0, decryptCFBKuz(close_text, open_text, key, 16, 32, (uint8_t *)initial_vector));
    fclose(open_text);
    fclose(close_text);
...
TEST(CFB, TestOnNotFullLastBlock) {
    FILE * open_text = fopen("OpenText.txt", "w");
    FILE * close_text;
    if(open_text == NULL)
    {
