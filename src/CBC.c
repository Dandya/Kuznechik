...
int encryptCBCKuz(FILE* input, FILE* output, vector128_t * key, int mode_padding_nulls, 
    int size_register_in_bytes, vector128_t * initial_vector) {
    if( key == NULL )                              { return -1; }
    if( input == NULL || output == NULL )          { return -2; }
    if( size_register_in_bytes % SIZE_BLOCK != 0 && size_register_in_bytes != 0 ) { return -3; }
...
int decryptCBCKuz(FILE* input, FILE* output, vector128_t * key, int mode_padding_nulls, int length_last_block, 
    int size_register_in_bytes, vector128_t * initial_vector)
{
    if( key == NULL )                                        { return -1; }
    if( input == NULL || output == NULL )                    { return -2; }
