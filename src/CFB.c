...
vector128_t getBlock(uint8_t * regist, int size_regist_in_bytes, int index_gamma_in_register, int size_block_in_bytes) {
...
int encryptCFBKuz(FILE* input, FILE* output, vector128_t * key, int size_block_in_bytes, int size_register_in_bytes, uint8_t * initial_vector) {
    if( key == NULL )                              { return -1; }
    if( input == NULL || output == NULL )          { return -2; }
    if( size_block_in_bytes > 16 || size_block_in_bytes <= 0 ) { return -3; }
    if( size_register_in_bytes < SIZE_BLOCK )      { return -4; }
...
int decryptCFBKuz(FILE* input, FILE* output, vector128_t * key, int size_block_in_bytes, int size_register_in_bytes, uint8_t * initial_vector) {
    if( key == NULL )                              { return -1; }
    if( input == NULL || output == NULL )          { return -2; }
    if( size_block_in_bytes > 16 || size_block_in_bytes <= 0 ) { return -3; }
    if( size_register_in_bytes <  SIZE_BLOCK )      { return -4; }
