...
#else
static uint8_t mulGf256_matrix[16][256] = {
...
#else
            result ^= mulGf256_matrix[i][block->bytes[i]];
#endif
...
#else
        block->bytes[15] = mulGf256_matrix[15][block->bytes[15]] ^ result;
#endif
    }
    else
    { // mode == DECRYPT
...
#else
        block->bytes[0] = mulGf256_matrix[0][byte] ^ result;
#endif
    }
}
...
/// @brief Function encrypts block of data in memory using base algorithm from GOST 34.12-2018.
...
/// @brief Function decrypts block of data in memory using base algorithm from GOST 34.12-2018.
/// @param block is pointer of data in memory for enc. Size block is 16 bytes.
/// @param arr_keys is array of iteration keys. Size of key is 128 bits.
/// @return 0 is good, -1 is block = NULL, -2 is arr_keys = NULL.
...
void procPaddingNulls(uint8_t *data, int count_adding_bytes, int mode)
{
    ...
    {
        for (int index = SIZE_BLOCK - count_adding_bytes; index < SIZE_BLOCK; index++)
        {
            data[index] = 0;
        }
    }
    else
    { // for PROC_ADD_NULLS_2 and PROC_ADD_NULLS_3
        data[SIZE_BLOCK - count_adding_bytes] = 1;
        for (int index = SIZE_BLOCK - (count_adding_bytes - 1); index < SIZE_BLOCK; index++)
        {
            data[index] = 0;
        }
    }
